                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend; i++)
    {
        if(vMatches12[i]>=0)
        {
            mvMatches12.push_back(make_pair(i,vMatches12[i]));// mvMatches12记录匹配上的特征点对
            mvbMatched1[i]=true;   // mvbMatched1记录每个特征点是否有匹配的特征点，
        }
        else
            mvbMatched1[i]=false;
    }

    const int N = mvMatches12.size();//匹配上的特征点个数
    ...
    for(int i=0; i<N; i++)
    {
        vAllIndices.push_back(i);// 新建一个容器vAllIndices，生成0到N-1的数作为特征点的索引
    }
    ...
    //计算homography和fundamental matrix并打分
    //ref的用法???
